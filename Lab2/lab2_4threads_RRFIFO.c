...
int main(int argc, char * argv[]) {
    ...
    int thread_s2, thread_s1,thread_s3,thread_s4;
    int t_cpu_s1, t_cpu_s2,t_cpu_s3,t_cpu_s4;
    cpu_set_t cpuset;
    int core_num;
    int policy;
    int sched_state_1,sched_state_2,sched_state_3,sched_state_4;
    ...
    pthread_barrier_init(&barr,NULL,16);
...
//    printf("The things is %s",scheduler);
    ...
    //    paramone.sched_priority = sched_get_priority_max(policy);
  //      struct sched_param paramtwo;
//        paramtwo.sched_priority = sched_get_priority_min(policy);
        //set cpu param
        CPU_ZERO(&cpuset);
        CPU_SET(core_num, &cpuset);
        
        // create two thread and sched them immediately
        thread_s1 = pthread_create( &thread[core_num*4], NULL, cube_function, (void*) message1);
        if(thread_s1)
        {
...
//     	printf("the value 1 is %d\n",param.sched_priority);
	param.sched_priority = midpriority;
        sched_state_1 = pthread_setschedparam(thread[core_num*4],policy,&param);
        if(sched_state_1)
        {
...
//	printf("the value 2 is %d\n",param.sched_priority);
	param.sched_priority = midpriority - 1;
        thread_s2 = pthread_create( &thread[core_num*4+1], NULL, cube_function, (void*) message2);
        if(thread_s2)
        {
            fprintf(stderr,"Error - pthread_create(thread2) return code: %d\n",thread_s2);
            return -1;
        }
        sched_state_2 = pthread_setschedparam(thread[core_num*4+1],policy,&param);
        if(sched_state_2)
        {
            fprintf(stderr,"Error - pthread_setschedparam(thread 2) return code: %d\n",sched_state_2);
        }
        thread_s3 = pthread_create( &thread[core_num*4+2], NULL, cube_function, (void*) message1);
        if(thread_s3)
        {
            fprintf(stderr,"Error - pthread_create(thread1) return code: %d\n",thread_s3);
            return -1;
...
//      printf("the value 1 is %d\n",param.sched_priority);
	param.sched_priority = midpriority + 1;
        sched_state_3 = pthread_setschedparam(thread[core_num*4+2],policy,&param);
        if(sched_state_3)
        {
            fprintf(stderr,"Error - pthread_setschedparam(thread3) return code: %d\n",sched_state_3);
        }
        thread_s4 = pthread_create( &thread[core_num*4+3], NULL, cube_function, (void*) message1);
        if(thread_s4)
        {
            fprintf(stderr,"Error - pthread_create(thread1) return code: %d\n",thread_s4);
            return -1;
...
//      printf("the value 1 is %d\n",param.sched_priority);
	param.sched_priority = sched_get_priority_max(policy);
        sched_state_4 = pthread_setschedparam(thread[core_num*4+3],policy,&param);
        if(sched_state_4)
        {
            fprintf(stderr,"Error - pthread_setschedparam(thread4) return code: %d\n",sched_state_4);
        }

        // set affinity
        t_cpu_s1 = pthread_setaffinity_np(thread[core_num*4], sizeof(cpu_set_t), &cpuset);
        if (t_cpu_s1 != 0)
        {
            fprintf(stderr,"Error - pthread_setaffinity_np(thread1) return code: %d\n",t_cpu_s1);
        }
        t_cpu_s2 = pthread_setaffinity_np(thread[core_num*4+1], sizeof(cpu_set_t), &cpuset);
        if (t_cpu_s2 != 0)
        {
            fprintf(stderr,"Error - pthread_setaffinity_np(thread2) return code: %d\n",t_cpu_s2);
        }
        t_cpu_s3 = pthread_setaffinity_np(thread[core_num*4+2], sizeof(cpu_set_t), &cpuset);
        if (t_cpu_s3 != 0)
        {
            fprintf(stderr,"Error - pthread_setaffinity_np(thread1) return code: %d\n",t_cpu_s3);
        }
        t_cpu_s4 = pthread_setaffinity_np(thread[core_num*4+3], sizeof(cpu_set_t), &cpuset);
        if (t_cpu_s4 != 0)
        {
            fprintf(stderr,"Error - pthread_setaffinity_np(thread2) return code: %d\n",t_cpu_s4);
        }

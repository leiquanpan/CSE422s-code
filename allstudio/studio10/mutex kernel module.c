...
{	
	...
	//time_t x = time(0);
	int x = count++;
	printk(KERN_INFO"share_data at the beginning in %d: %d",x,share_data);
        int i = 0;
	for (i=0; i<iters;i++){
	...
	printk(KERN_INFO"share data at the end in %d: %d",x,share_data);
	return 0;
}
/*
...
*/
    printk(KERN_INFO "The shared_data is: %d", share_data);

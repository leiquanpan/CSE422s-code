...
int Prime_function(int local_count_pos)
{
    ...
    while(pointer_to_array < upper_bound - 1)
    {
        mutex_lock(&mutex_for_shared);
        if(pointer_to_array > upper_bound - 2)
        {
	    mutex_unlock(&mutex_for_shared);
	    return 0;
	}
        local_variables = share_data[pointer_to_array++];
        mutex_unlock(&mutex_for_shared);
	...
	printk(KERN_ALERT"The locate is %d",local_count_pos);
    }
    return 1;
}
...
static int simple_init (void) {
    ...
    share_data = (int *)kmalloc((upper_bound - 1)*sizeof(int), GFP_KERNEL);
    for(;i <= upper_bound; i++)
    {
        share_data[i - 2] = i;
    }
...
static void simple_exit (void) {
    ...
    for(; i <= upper_bound - 2; i++)
    {
        if(share_data[i])
        {
            printk(KERN_ALERT "%d  ",share_data[i]);
            num++;
            if(num % 8 == 0)
                printk(KERN_ALERT "\n");
        }
